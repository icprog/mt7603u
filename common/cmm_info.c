...
RTMP_STRING *GetEncryptType(CHAR enc)
{
    if(enc == Ndis802_11WEPDisabled)
        return "NONE";
    else if(enc == Ndis802_11WEPEnabled)
    	return "WEP";
    else if(enc == Ndis802_11TKIPEnable)
    	return "TKIP";
    else if(enc == Ndis802_11AESEnable)
    	return "AES";
	else if(enc == Ndis802_11TKIPAESMix)
    	return "TKIPAES";
#ifdef WAPI_SUPPORT
	else if(enc == Ndis802_11EncryptionSMS4Enabled)
    	return "SMS4";
#endif /* WAPI_SUPPORT */
    else
    	return "UNKNOW";
...
RTMP_STRING *GetAuthMode(CHAR auth)
{
    if(auth == Ndis802_11AuthModeOpen)
    	return "OPEN";
    else if(auth == Ndis802_11AuthModeShared)
    	return "SHARED";
	else if(auth == Ndis802_11AuthModeAutoSwitch)
    	return "AUTOWEP";
    else if(auth == Ndis802_11AuthModeWPA)
    	return "WPA";
    else if(auth == Ndis802_11AuthModeWPAPSK)
    	return "WPAPSK";
    else if(auth == Ndis802_11AuthModeWPANone)
    	return "WPANONE";
    else if(auth == Ndis802_11AuthModeWPA2)
    	return "WPA2";
    else if(auth == Ndis802_11AuthModeWPA2PSK)
    	return "WPA2PSK";
	else if(auth == Ndis802_11AuthModeWPA1WPA2)
    	return "WPA1WPA2";
	else if(auth == Ndis802_11AuthModeWPA1PSKWPA2PSK)
    	return "WPA1PSKWPA2PSK";
#ifdef WAPI_SUPPORT
	else if(auth == Ndis802_11AuthModeWAICERT)
    	return "WAI-CERT";
	else if(auth == Ndis802_11AuthModeWAIPSK)
    	return "WAI-PSK";
#endif /* WAPI_SUPPORT */
	else
    	return "UNKNOW";
	...
	IN	ULONG			BufLen)
{
	...
	snprintf(pBuf, BufLen, "\n");
	for (i=0; i<4; i++){
	snprintf(pBuf+strlen(pBuf), BufLen-strlen(pBuf), "Filter Src IP address[%d]: %d\n"
			,i ,pAd->WOW_Cfg.FilterSrcIpAddr[i]);
		}
	
	return 0;
}

INT Show_WOWFilter_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT RTMP_STRING *pBuf,
	IN	ULONG			BufLen)
{
	snprintf(pBuf, BufLen, "\n");
	snprintf(pBuf+strlen(pBuf), BufLen-strlen(pBuf), "Waker MAC filter Setting: %d\n"
			, pAd->WOW_Cfg.waker_mac_check);
	snprintf(pBuf+strlen(pBuf), BufLen-strlen(pBuf), "Waker PW filter Setting: %d\n"
			, pAd->WOW_Cfg.waker_pw_check);
	return 0;
}



INT Show_WOWPATTERN_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT RTMP_STRING *pBuf,
	IN	ULONG			BufLen)
{
	snprintf(pBuf, BufLen, "\n");
	snprintf(pBuf+strlen(pBuf), BufLen-strlen(pBuf), "Pattern: %d\n"
			, pAd->WOW_Cfg.Pattern);
	return 0;
}

INT	Show_E2PMAC_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	RTMP_STRING *pBuf,
	IN	ULONG			BufLen)
{
...
#endif /* RELEASE_EXCLUDE */
	DBGPRINT(RT_DEBUG_OFF, ("==>RTMPIoctlE2PROM\n"));
	for(l=0;l<3;l++)
	{
		/* allocate memory */
...
#ifdef RELEASE_EXCLUDE
			DBGPRINT(RT_DEBUG_OFF, ("this_char=%s\n", this_char));
#endif /* RELEASE_EXCLUDE */ 	


#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_OFF, ("Read: this_char=%s, strlen=%zu\n", this_char, strlen(this_char)));
#endif /* RELEASE_EXCLUDE */
				...
				if(strlen(this_char) == 4)
				{
					...
					if (eepAddr < 0xFFFF)
					{
						...
						mac[p-l]=(eepValue & 0x00ff);
						mac[p-l-1]=(eepValue & 0xff00)>>8;
						p--;
						DBGPRINT(RT_DEBUG_OFF, ("eepAddr=%x, eepValue=0x%x\n", eepAddr, eepValue));
		...
		/* Copy the information into the user buffer */
		snprintf(pBuf, BufLen, "\n");
		snprintf(pBuf+strlen(pBuf), BufLen-strlen(pBuf), "MACADDR = %02x:%02x:%02x:%02x:%02x:%02x"
				,mac[5], mac[4], mac[3],
					mac[2], mac[1], mac[0]);
...
#ifdef FW_SS_COMMAND
	{"SwowInshowPattern", 	Show_WOWPATTERN_Proc},
	{"SwowInshowFilter", 	Show_WOWFilter_Proc},
	{"SwowInshowFilterSrcIP", 	Show_WOWFilter_SrcIP_Proc},
	{"E2PMAC", 				Show_E2PMAC_Proc},
#endif	
	{NULL, NULL}
};
