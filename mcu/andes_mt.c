...
#ifdef FW_SS_COMMAND
INT32 CmdCNN5WowSetFilterIP(RTMP_ADAPTER *pAd, UINT8 *IP)
{
	struct cmd_msg *msg;
	INT32 ret = 0;	
	EXT_CMD_CNN5_CTRL_T CmdCNN5Ctrl;
	//int i;
	NdisZeroMemory(&CmdCNN5Ctrl, sizeof(EXT_CMD_CNN5_CTRL_T));

	msg = AndesAllocCmdMsg(pAd, sizeof(EXT_CMD_CNN5_CTRL_T));
	if (!msg)
	{
		ret = NDIS_STATUS_RESOURCES;
		goto error;
	}
	AndesInitCmdMsg(msg, P1_Q0, EXT_CID, CMD_QUERY, EXT_CMD_ID_CCN5_WOW_Setting, 
					FALSE, 0, FALSE, FALSE, 0, NULL, NULL);

	NdisCopyMemory(CmdCNN5Ctrl.ucFilterSrcIP, IP, 4);	
	//for(i=0;i<4;i++)
	//	DBGPRINT(RT_DEBUG_OFF, ("CmdCNN5Ctrl.ucFilterSrcIP[%d]=%d\n", i, CmdCNN5Ctrl.ucFilterSrcIP[i]));
	
	AndesAppendCmdMsg(msg, (char *)&CmdCNN5Ctrl, sizeof(CmdCNN5Ctrl));
	ret = AndesSendCmdMsg(pAd, msg);
...
INT32 CmdCNN5WowSetAssistent(RTMP_ADAPTER *pAd, UINT8 Vaule)
{
	struct cmd_msg *msg;
	INT32 Ret = 0;
	EXT_CMD_CNN5_CTRL_T CmdCNN5Ctrl;
	...
	NdisZeroMemory(&CmdCNN5Ctrl, sizeof(EXT_CMD_CNN5_CTRL_T));
	msg = AndesAllocCmdMsg(pAd, sizeof(CmdCNN5Ctrl));
	...
	memset(&CmdCNN5Ctrl, 0x00, sizeof(CmdCNN5Ctrl));
	
	CmdCNN5Ctrl.ucAssistent = Vaule;
	AndesAppendCmdMsg(msg, (char *)&CmdCNN5Ctrl, 
								sizeof(CmdCNN5Ctrl));
	Ret = AndesSendCmdMsg(pAd, msg);
...
static NDIS_STATUS AndesMTLoadFwMethod1(RTMP_ADAPTER *ad)
{
	...
	if (cap->load_code_method == BIN_FILE_METHOD) {
		DBGPRINT(RT_DEBUG_OFF, ("load fw by request_firmware %s\n", cap->fw_bin_file_name));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	ret = AndesSendCmdMsg(pAd, msg);
#ifdef FW_SS_COMMAND	
	CmdCNN5WowSetFilterIP(pAd,pAd->WOW_Cfg.IpAddr);
#endif
#if 0
	/* ARP offload */
...
#ifdef FW_SS_COMMAND
	CmdPFGlobal.extParam = cpu2le32(pAd->WOW_Cfg.SSwowMode);
	CmdPFGlobal.resv[0] = (cpu2le32(pAd->WOW_Cfg.Pattern)); //bit 0 for Type A or Tyep B setting 
	CmdPFGlobal.resv[0] = CmdPFGlobal.resv[0] + (cpu2le32(pAd->WOW_Cfg.waker_mac_check) << 1);
	CmdPFGlobal.resv[0] = CmdPFGlobal.resv[0] + (cpu2le32(pAd->WOW_Cfg.waker_pw_check) << 2);
#endif
