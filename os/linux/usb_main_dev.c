...
#ifdef MODULE_PARAM_PID_VID
extern USB_DEVICE_ID rtusb_external_dev_id[];
extern INT const rtusb_external_usb_id_len;
UINT g_param_dev_vid = 0x0;
UINT g_param_dev_pid = 0x0;
#endif /* MODULE_PARAM_PID_VID */

#ifdef MODULE_PARAM_COUNTRY_REGION
UINT g_param_country_region = 99; //defaul value with only for indentification purpose
#endif /* MODULE_PARAM_COUNTRY_REGION */

#ifdef MODULE_PARAM_PID_VID
module_param_named(pid, g_param_dev_pid, uint, 0);
module_param_named(vid, g_param_dev_vid, uint, 0);
#endif /* MODULE_PARAM_PID_VID */

#ifdef MODULE_PARAM_COUNTRY_REGION
module_param_named(country_region, g_param_country_region, uint, 0);
#endif /* MODULE_PARAM_COUNTRY_REGION */
...
#ifdef MODULE_PARAM_PID_VID
struct usb_driver rtusb_external_driver = {
	.name = RTMP_DRV_NAME,
	.probe = rtusb_probe,
	.disconnect = rtusb_disconnect,
	.id_table = rtusb_external_dev_id,
};
#endif /* MODULE_PARAM_PID_VID */
...
INT __init rtusb_init(void)
{
...
#ifdef MODULE_PARAM_COUNTRY_REGION
	if(g_param_country_region != 99)
	{
		DBGPRINT(RT_DEBUG_OFF, ("###CountryRegion:%d module param received###\n", 
			g_param_country_region));
	}
#endif /* MODULE_PARAM_COUNTRY_REGION */

#ifdef MODULE_PARAM_PID_VID
	if(g_param_dev_pid != 0x0 && g_param_dev_vid != 0x0)
	{
		DBGPRINT(RT_DEBUG_OFF, ("###PID/VID:%x/%x module param received###\n", 
			g_param_dev_pid, g_param_dev_vid));
		rtusb_external_dev_id[0].idVendor = g_param_dev_vid;
		rtusb_external_dev_id[0].idProduct = g_param_dev_pid;
	}
#endif /* MODULE_PARAM_PID_VID */
	...
	if(g_param_dev_pid!=0x0 && g_param_dev_vid!=0x0)
	{
		return usb_register(&rtusb_external_driver);
	}
#endif /* MODULE_PARAM_PID_VID */
...
VOID __exit rtusb_exit(void)
{
	...
	if(g_param_dev_pid!=0x0 && g_param_dev_vid!=0x0)
	{
		return usb_deregister(&rtusb_external_driver);
	}
#endif /* MODULE_PARAM_PID_VID */
	return usb_deregister(&rtusb_driver);
