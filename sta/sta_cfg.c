...
#ifdef FW_SS_COMMAND
	{"SwowInStart",					Set_WOW_InStart},
	{"SwowInSetFilter",				Set_WOWInFilter},
	{"SwowInSetPattern",			Set_WOW_Pattern},
	{"SwowInSetFilterSrcIP",		Set_WOW_Filter_Src_IP},
	{"SwowInSetAssistant",			Set_WOW_Assistant},
#endif
...
#ifdef MT_WOW_SUPPORT
#ifdef FW_SS_COMMAND
INT Set_WOW_InStart(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	int i=0;
	...
	while ((this_char = strsep((char **)&arg, ".,")) != NULL) {
		...
		if(i<4){
			pAd->WOW_Cfg.IpAddr[i] = simple_strtol(this_char, 0, 10);
			i++;
			}
		else{
			pAd->WOW_Cfg.SSwowMode = simple_strtol(this_char, 0, 10);
			}
		}
...
INT Set_WOW_Pattern(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	ULONG Value = simple_strtol(arg, 0, 10);
	if((Value == 0) || (Value == 1)){ //1: Type B
		pAd->WOW_Cfg.Pattern = ((UINT8)Value);
		}
	else{
		pAd->WOW_Cfg.Pattern = 0; // Type A
		}
	DBGPRINT(RT_DEBUG_ERROR, ("Pattern = %d\n", pAd->WOW_Cfg.Pattern));
	return TRUE;	
}

INT Set_WOW_Filter_Src_IP(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	int i=0;
	RTMP_STRING *this_char;	

	while ((this_char = strsep((char **)&arg, ".")) != NULL) {
		DBGPRINT(RT_DEBUG_OFF, ("Filter_Src_IP = %s\n", this_char));
		if(i<4){
			pAd->WOW_Cfg.FilterSrcIpAddr[i] = simple_strtol(this_char, 0, 10);
			i++;
			}
		}
	CmdCNN5WowSetFilterIP(pAd, pAd->WOW_Cfg.FilterSrcIpAddr);

	return TRUE;	
}


INT Set_WOW_Assistant(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	ULONG Value = simple_strtol(arg, 0, 10);
	if((Value == 0) || (Value == 1)){ 
		pAd->WOW_Cfg.Assistant = ((UINT8)Value);
		}
	else{
		pAd->WOW_Cfg.Assistant = 0;
		}
	DBGPRINT(RT_DEBUG_ERROR, ("Assistant = %d\n", pAd->WOW_Cfg.Assistant));
	CmdCNN5WowSetAssistent(pAd, (UINT8)pAd->WOW_Cfg.Assistant);
	return TRUE;	
}


INT Set_WOWInFilter(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	RTMP_STRING *this_char;
	int i=0;
	while ((this_char = strsep((char **)&arg, ",")) != NULL) {
		if(i == 0){
			pAd->WOW_Cfg.waker_mac_check = simple_strtol(this_char, 0, 10);
			i++;
			}
		else{
			pAd->WOW_Cfg.waker_pw_check = simple_strtol(this_char, 0, 10);
			}
		}
	DBGPRINT(RT_DEBUG_OFF, ("pAd->WOW_Cfg.waker_pw_check = %d\n", pAd->WOW_Cfg.waker_pw_check));
	DBGPRINT(RT_DEBUG_OFF, ("pAd->WOW_Cfg.waker_mac_check = %d\n", pAd->WOW_Cfg.waker_mac_check));
	return TRUE;	
}
#endif
